...
extract_RCnorm_sig <- function(tmp, num.rand = 2, sig.level = 0.05){
  ...
  rc.res <- matrix(nrow = 150, ncol = 5) %>% as.data.frame()
  colnames(rc.res) <- c("num", "family.codes", "Club Size", "Rich Club", "p.value")
  
  rc.res$num <- tmp$num %>% rep(150)
  rc.res$family.codes <- tmp$family.codes %>% rep(150)
  rc.res$`Club Size` <- 1:150
