...
# writing the update and rewire again together here -----------------------
  ...
  # starting_update <- brain_growing@age$update + 1
  # owner_firstname <- strsplit(x = brain_growing@name, split = " ")[[1]][1]
  # 
  # time_start <- Sys.time()
  # for(m in 1: n_rewires) {
  #   for(h in starting_update:(starting_update+n_hr)){
  #     brain_growing <- brain_growing %>% heartupdate_update()
  #   }
  #   brain_growing <- brain_growing %>%
  #     heartupdate_rewire()#freq_snapshot = toy_brain@parameters$freq_snapshot)
  #   if(!quiet) print(paste(owner_firstname, "is", m, "rewires old now."))
  # }
  # 
